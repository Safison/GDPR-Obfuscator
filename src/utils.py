...
def parse_input_json(input_json):
    ...
    try:
        if input_json == {}:
            ...
            if file_to_obfuscate and not file_to_obfuscate.startswith("s3://") or file_to_obfuscate.count("/") < 2:
                return "Invalid S3 URI format", "", []
            if not (file_to_obfuscate.endswith(".csv") or file_to_obfuscate.endswith(".parquet") or file_to_obfuscate.endswith(".json")):
...
#          "file_to_obfuscate": "s3://ans-gdpr-bucket/studentss.json",
#          "pii_fields": []
#       })
